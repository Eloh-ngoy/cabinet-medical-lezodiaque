...
<head>
    ...
    <title>Modifier patient - <?= APP_NAME ?></title>
...
<body>
    ...
    <main class="main-content">
        ...
        <form class="card" method="POST" action="<?= url('patients/update') ?>">
            <input type="hidden" name="csrf_token" value="<?= generateCSRFToken() ?>">
            <input type="hidden" name="id" value="<?= $patient['id'] ?>">
            <div class="form-grid">
                <div>
                    <label for="nom">Nom</label>
                    <input id="nom" name="nom" value="<?= htmlspecialchars($patient['nom']) ?>" required>
                </div>
                <div>
                    <label for="prenom">Prénom</label>
                    <input id="prenom" name="prenom" value="<?= htmlspecialchars($patient['prenom']) ?>" required>
                </div>
                <div>
                    <label for="telephone">Téléphone</label>
                    <input id="telephone" name="telephone" value="<?= htmlspecialchars($patient['telephone']) ?>" required>
                </div>
                <div>
                    <label for="email">Email</label>
                    <input id="email" name="email" type="email" value="<?= htmlspecialchars($patient['email']) ?>" required>
                </div>
                <div>
                    <label for="date_naissance">Date de naissance</label>
                    <input id="date_naissance" name="date_naissance" type="date" value="<?= htmlspecialchars($patient['date_naissance']) ?>" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="groupe_sanguin" name="groupe_sanguin" value="<?= htmlspecialchars($patient['groupe_sanguin'] ?? '') ?>">
                </div>
                <div>
                    <label for="statut_interne_externe">Statut</label>
                    <select id="statut_interne_externe" name="statut_interne_externe">
                        <option value="externe" <?= $patient['statut_interne_externe'] === 'externe' ? 'selected' : '' ?>>Externe</option>
                        <option value="interne" <?= $patient['statut_interne_externe'] === 'interne' ? 'selected' : '' ?>>Interne</option>
                    </select>
                </div>
                <div style="grid-column: 1 / -1;">
                    <label for="traitement_passe">Traitement passé</label>
                    <textarea id="traitement_passe" name="traitement_passe"><?= htmlspecialchars($patient['traitement_passe'] ?? '') ?></textarea>
                </div>
            </div>
            <div class="actions">
