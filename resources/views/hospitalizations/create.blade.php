...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-4">Nouvelle Hospitalisation</h1>
    </div>

    <form method="POST" action="{{ route('hospitalizations.store') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Lit disponible</label>
            ...
            <div>
                ...
                <input type="datetime-local" name="admission_date" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('admission_date') }}">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Durée prévue (jours)</label>
                <input type="number" name="expected_duration" min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value="{{ old('expected_duration') }}">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Motif d'admission</label>
                <textarea name="admission_reason" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('admission_reason') }}</textarea>
            </div>
        </div>
        ...
        <div class="mt-6 flex justify-end gap-4">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Créer l'hospitalisation</button>
        </div>
    </form>
@endsection
