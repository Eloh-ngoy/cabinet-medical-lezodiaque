...
<head>
    ...
    <title>Rapport d'Audit du Patient</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p class="document-title">Rapport d'Audit du Patient</p>
            <p class="document-id" style="color: #64748b; margin: 5px 0 0 0;">Document ID: {{ $document->document_id }}</p>
        </div>

        <div class="section">
            <h2 class="section-title">Patient Audité</h2>
            <div class="info-grid">
                <div class="info-row">
        ...
        <div class="section">
            <h2 class="section-title">Statistiques d'Audit</h2>
            <div class="info-grid">
                <div class="info-row">
                    <span class="info-label">Total des Modifications:</span>
                    <span class="info-value">{{ $activities->count() }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Date de Génération:</span>
                    <span class="info-value">{{ $document->generated_at->format('d/m/Y H:i') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            <div class="audit-item">
                <div class="audit-header">
                    <span>{{ $activity->causer->full_name ?? 'Système' }}</span>
                    <span>{{ $activity->created_at->format('d/m/Y H:i') }}</span>
                </div>
                <div class="audit-details">
                    <strong>Action:</strong> {{ $activity->description ?? 'Modification du dossier' }}<br>
                    <strong>IP:</strong> {{ $activity->properties['ip_address'] ?? 'Non disponible' }}
                    @if($activity->properties)
                    <br>
                    @foreach($activity->properties as $key => $value)
                        @if($key != 'ip_address' && $key != 'description')
            ...
            @empty
            <p style="color: #64748b; text-align: center; padding: 20px;">Aucune activité d'audit enregistrée pour ce patient</p>
            @endforelse
        </div>
        ...
        <div class="footer">
            <div>
                ...
                Total des modifications: {{ $activities->count() }}<br>
                <em>Ce document contient l'historique complet des modifications apportées au dossier patient.</em>
            </div>
