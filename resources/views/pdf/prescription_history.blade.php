...
<head>
    ...
    <title>Historique des Prescriptions</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p class="document-title">Historique des Prescriptions</p>
        ...
        <div class="section">
            <h2 class="section-title">Filtres Appliqués</h2>
            <div class="info-grid">
                @if($filters['start_date'])
                <div class="info-row">
                    <span class="info-label">Date Début:</span>
                    <span class="info-value">{{ $filters['start_date'] }}</span>
                </div>
                @endif
                @if($filters['end_date'])
                <div class="info-row">
                    <span class="info-label">Date Fin:</span>
                    <span class="info-value">{{ $filters['end_date'] }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="section">
            <h2 class="section-title">Historique des Consultations et Prescriptions</h2>
            <div class="info-row" style="font-weight: 700; color: #1e40af; margin-bottom: 15px;">
                <span>Total: {{ $consultations->count() }} consultation(s)</span>
            </div>
            
            @forelse($consultations as $consultation)
            <div class="prescription-item">
                <div class="prescription-header">
                    <span>Consultation du {{ $consultation->date_consultation->format('d/m/Y H:i') }}</span>
                ...
                <div class="prescription-content">
                    ...
                    <div style="margin-top: 10px; padding: 10px; background: white; border-radius: 4px;">
                        ...
                        <p style="white-space: pre-line; margin-top: 5px; font-size: 13px;">{{ $consultation->ordonnance }}</p>
                    </div>
                    @endif
                    <p style="margin-top: 10px;"><strong>Coût:</strong> {{ number_format($consultation->prix, 0) }} CDF</p>
                </div>
            </div>
            @empty
            <p style="color: #64748b; text-align: center; padding: 20px;">Aucune prescription trouvée pour la période sélectionnée</p>
            @endforelse
        </div>
        ...
        <div class="footer">
            <div>
                ...
                Total des prescriptions: {{ $consultations->count() }}
            </div>
