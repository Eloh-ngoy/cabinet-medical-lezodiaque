...
@section('content')
    ...
    <form method="POST" action="{{ route('pharmacy.store') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="category"
                    ...
                    @foreach($categories as $cat)<option value="{{ $cat }}" {{ old('category') === $cat ? 'selected' : '' }}>
                    {{ $cat }}</option>@endforeach
                </select>
            </div>
            <div>
        ...
        <div class="mt-6 flex justify-end gap-4">
            ...
            <button type="submit"
                class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Ajouter</button>
        </div>
    </form>
@endsection